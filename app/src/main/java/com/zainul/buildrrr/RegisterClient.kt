...
import com.zainul.buildrrr.databinding.ActivityRegisterClientBinding
import kotlinx.android.synthetic.main.activity_register_client.*


class RegisterClient : Fragment() {

    private lateinit var auth: FirebaseAuth
    private lateinit var database: FirebaseDatabase
    private var _binding: ActivityRegisterClientBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = ActivityRegisterClientBinding.inflate(inflater, container, false)
        return binding.root
            ...
            Toast.makeText(activity, "Masukan Detail Yang Valid", Toast.LENGTH_SHORT).show()
                ...
                auth.createUserWithEmailAndPassword(email, password)
                    .addOnCompleteListener {
                        if (it.isSuccessful) {
                            val databaseRef =
                                database.reference.child("Client").child(auth.currentUser!!.uid)
